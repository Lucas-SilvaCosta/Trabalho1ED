...
template<typename T>
structures::ArrayQueue<T>::ArrayQueue() { // default constructor
...
template<typename T>
structures::ArrayQueue<T>::ArrayQueue(std::size_t max) {  // contructor with parameters
    max_size_ = max;
    contents = new T[max];
    size_ = 0;
    begin_ = 0;
...
template<typename T>
structures::ArrayQueue<T>::~ArrayQueue() {  // destructor
    delete [] contents;
}
